...
# import sys
# sys.path
# sys.path.append('/Users/yalinli_cabbi/OneDrive/Coding/bst_modules/webapp')
...
def get_demo_results():
    ...
    print('\n--------- Solving for overall organic loading rate ---------')
    OLR_all = solve_param_from_rm(param='OLR_all', bounds=(0, 35/24), OLR_all=None,
                                  v=3, V_ratio=1.5, waste_ratio=0.05, COD_rm=0.918)
    print(f'\nOLR_all is {OLR_all:.3f}.')
    design = get_design()
    print(design)

    print('\n--------- Solving for bottom-to-top rx volume ratio ---------')
    ...
    print('\n--------- Solving for waste sludge ratio ---------')
...
OLRs = np.arange(start=0.5, stop=35/24, step=0.005)
waste_ratios = np.arange(start=0, stop=1, step=0.005)
...
def evalute_IC_design(OLRs, waste_ratios, V_ratios,
                      upstream_inputs=(), excel_name=None,
                      dropna=True):
    if upstream_inputs:
        all_rx['Qi'], all_rx['SS'], all_rx['X'] = upstream_inputs
    dfs = []
    for r in V_ratios:
    ...
    if excel_name:
        ...
        for i, df in enumerate(dfs):
            df.dropna().to_excel(writer, sheet_name=f'V_ratio={V_ratios[i]}')
        writer.save()
    return dfs


# To save max COD removal at each given value
def save_max_COD(dfs, filter_by='', excel_name=None):
    max_dfs = []
    for df in dfs:
        df = df.dropna()
        max_df = pd.DataFrame()
        filter_col = getattr(df, filter_by)
        for val in filter_col.unique():
            max_data = df[df.COD_rm.eq(df[filter_col.eq(val)].COD_rm.max())]
            max_df = max_df.append(max_data)
        max_dfs.append(max_df)
    if excel_name:
        writer = pd.ExcelWriter(path=excel_name)
        for i, max_df in enumerate(max_dfs):
            max_df.to_excel(writer, sheet_name=f'V_ratio={V_ratios[i]}')
        writer.save()
    return max_dfs
...
# max_OLR_dfs = save_max_COD(dfs, 'OLR', excel_name='COD_rm_max_OLR.xlsx')
# max_waste_ratio_dfs = save_max_COD(dfs, 'waste_ratio',
#                                    excel_name='COD_rm_max_waste_ratio.xlsx')
...
# Try some other assumptions
# upstream_inputs = (325, 6, 0.23)
# dfs = evalute_IC_design(OLRs, waste_ratios, V_ratios)
# fig, axes = plot_COD_rm(dfs, waste_ratios, OLRs, nplts=len(V_ratios), ncols=3)


# COD_rm = get_cod_rm(mu_max=0.01, Y=0.07, F_xb=0.0032, F_xt=0.0281,
#                     OLR_all=1.084, V_ratio=1, v=3, waste_ratio=0.055)

# waste_ratio = solve_param_from_rm(param='waste_ratio', bounds=(0, 1), waste_ratio=None,
#                                   OLR_all=1.084, V_ratio=1, v=3, COD_rm=0.82)
...
def get_yield_dct(chemicals):
    '''
    Get dicts of theoretical gas product yields as product/mol chemical.
    '''
    yield_dct = {
        'CH4': {},
        'CO2': {},
...
# Calculated is 352 kmol/hr, cs.R601.outs[0].imol['CH4'] is around 328 kmol/hr
...
def get_gas_volume(stream, yield_array):
    ...
    for ID, array in yield_array.items():
        ...
        chemical = getattr(chems, ID)
        if chemical.locked_state:
            V = chemical.V(T=35+298.15, P=101325) # in m3/mol
        else:
            V = chemical.V(T=35+298.15, P=101325, phase='g') # in m3/mol
        vol += (stream.mol*array).sum()*V*1e3 # stream.mol in kmol/hr
    return vol

# Calculated is 19363 m3/hr, cs.R601.outs[0].F_vol is around 18560 m3/hr
